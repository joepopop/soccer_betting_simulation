...
unbiased_arbitrage_compounded <- readxl::read_excel("data/processed/unbiased_arbitrage.xlsm") %>% 
  mutate(
    wager_h = round(compounded_wager_h, 2),
    wager_d = round(compounded_wager_d, 2),
    wager_a = round(compounded_wager_a, 2),
    wager_total = round(compounded_wager_total, 2),
    profit = round(compounded_profit, 2),
    cum_profit = round(compounded_cum_profit, 2)
    ) %>% 
  select(date, wager_h, wager_d, wager_a, wager_total, profit, cum_profit) 
